...
const Home = ({ location, match }) => {
  ...
  const fetchAuthorQuotes = (id) => {
    fetch(
      `https://quotes-hub.herokuapp.com/api/quotes/v2/source/${id}/quotes/?page=${page}&size=30`
    )
      .then((res) => res.json())
      .then(
        ({ quotes, count }) => {
  ...
  const fetchCategory = (id) => {
    fetch(
      `https://quotes-hub.herokuapp.com/api/quotes/v2/category/${id}/?page=${page}&size=30`
    )
      .then((res) => res.json())
      .then(
        ({ quotes, count }) => {
  ...
  const fetchHome = () => {
    console.log(page);
    fetch(
      `https://quotes-hub.herokuapp.com/api/quotes/v3/home/?featured=true&page=${page}&size=30`
    )
      .then((res) => res.json())
      .then(
        ({ quotes, count }) => {
